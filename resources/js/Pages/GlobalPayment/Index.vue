    ...
    <AppLayout title="Abonos">
        <div class="px-2 lg:px-10 py-7">
            ...
            <div class="flex justify-between items-center mx-3">
                <h1 class="font-bold text-lg">Registro de abonos</h1>
                <div class="relative">
                    <!-- filtro -->
            ...
            <div v-else class="mt-8 lg:w-11/12">
                <p v-if="localGolabalPayments.length" class="text-gray66 text-[11px]">{{ localGolabalPayments.length }} de {{ total_global_payments }}
                    elementos
                </p>
                <GlobalPaymentsTable :global_payments="localGolabalPayments" class="" />
                <!-- <GlobalPaymentsMobileIndex v-for="item in localGolabalPayments" :key="item.id" :saleId="item.id" class="md:hidden" /> -->
                <p v-if="localGolabalPayments.length" class="text-gray66 text-[11px]">{{ localGolabalPayments.length }} de {{ total_global_payments }}
                    elementos
                </p>
                ...
                <button v-else-if="total_global_payments > 20 && (localGolabalPayments.length < total_global_payments) && localGolabalPayments.length"
...
import GlobalPaymentsTable from '@/Components/MyComponents/GlobalPayment/GlobalPaymentsTable.vue';
import GlobalPaymentsMobileIndex from '@/Components/MyComponents/GlobalPayment/GlobalPaymentsMobileIndex.vue';
...
export default {
    data() {
        return {
            loading: false,
            localGolabalPayments: this.global_payments,
    ...
    props: {
        ...
        clients: Array,
        total_global_payments: Number
    },
    methods: {
        async searchSales() {
            this.loading = true;
            try {
                const response = await axios.get(route('global-payments.search'), { params: { queryDate: this.searchDate, queryClient: this.searchClient } });
                if (response.status == 200) {
                    this.localGolabalPayments = response.data.items;
                }
        ...
        async fetchItemsByPage() {
            try {
                ...
                const response = await axios.get(route('global-payments.get-by-page', this.currentPage));

                if (response.status === 200) {
                    this.localGolabalPayments = [...this.localGolabalPayments, ...response.data.items];
                    this.currentPage++;
                }
            } catch (error) {
                console.log(error)
