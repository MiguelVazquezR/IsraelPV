    ...
    <AppLayout :title="client.name">
        <div class="px-2 lg:px-10 py-7 text-xs lg:text-sm">
            ...
            <div class="grid grid-cols-3 lg:grid-cols-12 gap-1 my-7 ml-4">
                ...
                <p class="col-span-2 lg:col-span-11 bg-yellow-200 w-28">${{ totalDebt }}</p>
            </div>
            ...
            <el-tabs v-model="activeTab">
                ...
                <el-tab-pane label="Todas las ventas" name="2">
                    ...
                    <SaleDesktopDetail v-if="reversedSales?.length" :salesId="client.sales.map(item => item.id)" class="hidden md:block mb-2" />
    ...
    methods:{
        storePayment() {
            this.pendent_sales.forEach(sale => {
                 // Detener si no hay más cantidad para abonar
