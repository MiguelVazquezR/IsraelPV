<template>
    <div v-if="clients.length" class="w-full mx-auto text-[11px] md:text-sm overflow-auto">
        ...
        <div>
            <div v-for="client in clients" :key="client.id" class="*:p-3 h-12 cursor-pointer flex items-center space-x-4 border rounded-full mb-2 hover:border-primarylight" 
            @click="$inertia.get(route('clients.show', client.id))">
                ...
                <div class="rounded-r-full w-[17%] text-center">
                    <i @click.stop="$inertia.get(route('clients.edit', client.id))" class="fa-solid fa-pencil text-primary cursor-pointer hover:bg-gray-200 rounded-full mr-1 p-2"></i>
                    <el-popconfirm confirm-button-text="Si" cancel-button-text="No" icon-color="#C30303" title="¿Continuar?" @confirm="deleteItem(client.id)">
                        <template #reference>
